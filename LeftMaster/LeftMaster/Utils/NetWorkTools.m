...
+ (void)requestWithBean:(__kindof AJRequestBeanBase *)requestBean callBack:(AJRequestCallBack)callBack
{
    ...
    [url appendString:requestUrl];
    for (NSString *key in params) {
        [url appendString:[NSString stringWithFormat:@"&%@=%@",key,params[key]]];
    }
    ...
    switch ([requestBean httpMethod]) {
        ...
        {
            requestTask = [manager GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                __strong __typeof__(weakSelf) strongSelf = weakSelf;
        ...
        case HTTP_METHOD_POST:
        {
            ...
            if ([requestBean constructingBodyBlock] != nil) {
                ...
                // 文件等富文本内容
                requestTask = [manager POST:url parameters:nil constructingBodyWithBlock:[requestBean constructingBodyBlock] progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                    __strong __typeof__(weakSelf) strongSelf = weakSelf;
