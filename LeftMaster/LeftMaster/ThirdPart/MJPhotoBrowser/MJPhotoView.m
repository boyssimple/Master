...
//            }];
            ...
            [_imageView sd_setImageWithPreviousCachedImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageLowPriority|SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                if (receivedSize > kMinProgress) {
                    if (self.willHide != 1) {
                        loading.progress = (float)receivedSize/expectedSize;
                    }
...
//	}
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        _imageView.frame = CGRectMake((DEVICEWIDTH-100*RATIO_WIDHT320)/2.0, (DEVICEHEIGHT-200*RATIO_WIDHT320)/2.0, 100*RATIO_WIDHT320, 200*RATIO_WIDHT320);//[_photo.srcImageView convertRect:_photo.srcImageView.bounds toView:nil];
